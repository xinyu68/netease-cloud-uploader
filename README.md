...
将本地音频上传到用户自己的网易云音乐云盘，也可以取回当前账号云盘中的原始文件，并核对云盘记录、公开曲库匹配和文件信息。匹配缺失或错误时，可以搜索候选歌曲，让用户选择后纠正关联；确实没有可信曲库候选时，还可以按用户要求修改未匹配歌曲的内嵌封面或歌词。
...
> 把我网易云云盘里的《歌曲名》下载到我的下载目录

Skill 会先在当前账号的云盘记录中定位歌曲，不接受任意公开曲库 ID。下载采用临时文件，只有实际文件大小和 MD5 都与云盘原始记录一致时才落盘；不会覆盖同名文件，也不会把网易返回的转码版本冒充原文件。
