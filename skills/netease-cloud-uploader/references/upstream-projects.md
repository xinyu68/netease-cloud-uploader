...
当前账号云盘原文件下载使用直接依赖提供的 `song_download_url_v1`，并仅在主下载端点不可用时尝试 `song_url_v1`。2026-09-15 以未匹配的个人云盘 FLAC 记录做过真实验证：服务端返回文件的 MD5 和大小与 `privateCloud` 登记值完全一致。该结论不代表所有历史记录都一定保留原始字节，因此正式命令仍逐文件强制校验，不接受转码结果。
