...
description: 上传本地音频到用户自己的网易云音乐云盘、下载当前账号云盘中的原始文件、检查或按需修改内嵌封面和歌词，并纠正公开曲库关联。适用于用户本人云盘的上传、原文件取回、媒体信息修复和歌曲匹配管理；不用于下载任意公开歌曲、绕过付费或访问限制
---

# 网易云音乐云盘管理

通过 `scripts/ncm-cloud.js` 执行确定性的登录检查、上传、当前账号云盘原文件下载、曲库搜索和匹配纠正。底层使用网易客户端内部接口，接口可能变化；每次写操作后都回查实际状态，不把 HTTP 成功等同于业务成功。
...
只处理 `cloud-list` 能在当前登录账号中找到的个人云盘记录，不接受任意公开曲库歌曲 ID。先用 `cloud-list <关键词>` 定位唯一记录；不确定时展示候选并等待用户选择。用户确认记录和本地目标路径后，运行：

```powershell
node scripts/ncm-cloud.js cloud-download <cloudRecordId> <已有目录或完整文件路径> --yes
```

目标是目录时沿用云盘 `fileName`，目标是文件路径时使用用户指定名称。命令不覆盖已有文件，先写同目录临时文件；只有下载结果的 MD5 和大小都与云盘记录完全一致时才改名为最终文件。签名下载 URL 和 Cookie 不得输出。若网易只返回转码版本、链接不可用或校验不一致，删除临时文件并报告失败，不得把转码文件伪装成原文件。下载只改变本地文件系统，不修改云盘记录。
